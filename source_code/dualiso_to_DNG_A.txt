...
while grep -q 'CR2' /tmp/DUALISO/list_02
do

CR2=$(grep 'CR2' /tmp/DUALISO/list_02 | awk 'FNR == 1 {print}')
DNG=$(grep 'DNG' /tmp/DUALISO/list_02 | awk 'FNR == 1 {print}')
#Will add stored white balance settings to processed readymade dualiso DNG files
echo "$(tail -n +3 /tmp/DUALISO/list_02)" > /tmp/DUALISO/list_02
...
/Applications/Adobe\ DNG\ Converter.app/Contents/MacOS/Adobe\ DNG\ Converter -d /tmp/DUALISO/DNG_A "$CR2" 

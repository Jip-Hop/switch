...
#run dfort focus pixel script
    ...
    then
    fpm.sh -f -n -m crop_rec -o "$(cat /tmp/DUALISO/path_1)"/"$FILE_03o".$map "$(cat /tmp/DUALISO/path_1)"/"$FILE_03"
    else
    fpm.sh -f -n -o "$(cat /tmp/DUALISO/path_1)"/"$FILE_03o".$map "$(cat /tmp/DUALISO/path_1)"/"$FILE_03"
    fi
    else
    if ls /tmp/DUALISO/crop_rec
    then

...
#This program is free software; you can redistribute it and/or
 ...
 # Boston, MA  02110-1301, USA.
    ...
    while ! [ x"$(cat /tmp/DUALISO/MLV_cprsab)" = x ]
    do 
    FILE_02="$(cat /tmp/DUALISO/MLV_cprsab | head -1)"
#cut to the next name on the list
    echo "$(tail -n +2 /tmp/DUALISO/MLV_cprsab)" > /tmp/DUALISO/MLV_cprsab
#run MLV compressing
    if [ -f /tmp/DUALISO/MLV_cprs_output ]
    then
    $mlv_dump $set -o "$(cat /tmp/DUALISO/MLV_cprs_output)"/tmp_"$FILE_02" "$(cat /tmp/DUALISO/path_1)"/"$FILE_02"
    else
...
#delete or not
    ...
    if [ -f /tmp/DUALISO/MLV_cprs_output ]
    then
    mv -i "$(cat /tmp/DUALISO/MLV_cprs_output)"/tmp_"$FILE_02" "$(cat /tmp/DUALISO/MLV_cprs_output)"/"$FILE_02"
    else

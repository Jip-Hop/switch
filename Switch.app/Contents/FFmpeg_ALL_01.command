...
echo 1trap > /tmp/DUALISO/X_prores_TRAP
#first check for output
    ...
    then
    osascript -e 'display notification "exporting files to ProRes with proxies" with title "Auto mode"'
...
#determine if HDR or not
    ...
    cin=
    cinpr=
    ffmpeg -i "$name" -pix_fmt yuvj444p -r 1 -t 2 /tmp/DUALISO/check%03d.jpeg
    convert /tmp/DUALISO/check001.jpeg -colorspace hsb -resize 1x1 /tmp/DUALISO/check1.txt
    convert /tmp/DUALISO/check002.jpeg -colorspace hsb -resize 1x1 /tmp/DUALISO/check2.txt
    numb=$(echo $(grep '%' /tmp/DUALISO/check1.txt | cut -d "%" -f3 | tr -d ',') - $(grep '%' /tmp/DUALISO/check2.txt | cut -d "%" -f3 | tr -d ',') | bc -l)
    if (( $(echo "$numb > 5" |bc -l) )) || (( $(echo "$numb < -5" |bc -l) )) ; then
    echo "HDR" >> /tmp/_X_HDR
    else
    find /tmp/_X_HDR | xargs perl -pi -e 's/HDR//g'
    fi
    rm /tmp/DUALISO/check001.jpeg
    rm /tmp/DUALISO/check002.jpeg
    rm /tmp/DUALISO/check1.txt
    rm /tmp/DUALISO/check2.txt
...
#watch out for HDR footage
    if grep 'HDR\|halfhdr' /tmp/_X_HDR 
    then
    if grep 'copy' /tmp/_X_ProRes4444 
    then 
    cin= 
...
#watch ut for half fps(HDR setting)
    if grep -q 'halfhdr' /tmp/_X_HDR && grep -q 'HDR' /tmp/_X_HDR
    then
...
#remove trap
find /tmp/DUALISO/X_prores_TRAP | xargs perl -pi -e 's/1trap//g'

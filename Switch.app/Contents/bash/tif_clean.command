...
#Switch log not needed
rm LOG.txt
#log
rm "$(cat /tmp/DUALISO/path_1)"/TIFCLEAN_LOG.txt
...
echo outputpath: "$(cat /tmp/DUALISO/path_1)" >> TIFCLEAN_LOG.txt
echo applicationpath: "$(cat /tmp/DUALISO/path_2)" >> TIFCLEAN_LOG.txt
echo "" >> TIFCLEAN_LOG.txt
...
sleep 1
. "$(cat /tmp/DUALISO/path_2 | tr -d '"' )"bash/tif_clean.command
;;
esac
fi
...
#LOG
#log file
exec &> >(tee -a "$(cat /tmp/DUALISO/path_1)"/TIFCLEAN_LOG.txt >&2 )
cd "$(cat /tmp/DUALISO/path_1)"
...
#spit out tif files
"$(cat /tmp/DUALISO/path_2)"ffmpeg -i $(cat /tmp/TIFCLEAN | head -1) -pix_fmt rgb24 %06d.tif
else
...
wav=
acodec=
if ! [ x"$("$(cat /tmp/DUALISO/path_2)"ffmpeg -i $(ls *.{MOV,mov,mp4,MP4,mkv,MKV,avi,AVI} | head -1) 2>&1 | grep Audio)" = x ]
then 
"$(cat /tmp/DUALISO/path_2)"ffmpeg -i $(ls *.{MOV,mov,mp4,MP4,mkv,MKV,avi,AVI} | head -1) -vn -acodec copy $(ls *.{MOV,mov,mp4,MP4,mkv,MKV,avi,AVI} | head -1 | cut -d "." -f1).wav
wav=$(printf "%s\n" -i $(ls *.{MOV,mov,mp4,MP4,mkv,MKV,avi,AVI} | head -1 | cut -d "." -f1).wav)
acodec=$(printf "%s\n" -c:v copy -c:a aac)
fi
if ! [ x"$(ls *.mov)" = x ] || ! [ x"$(ls *.MOV)" = x ]
then
"$(cat /tmp/DUALISO/path_2)"ffmpeg $wav -r $(/usr/local/bin/exiftool $(ls *.{MOV,mov,mp4,MP4,mkv,MKV,avi,AVI} | head -1) | grep 'Video Frame Rate' | cut -d ":" -f2) -i %06d.tiff $acodec -vcodec prores -pix_fmt yuv422p10le "$(cat /tmp/DUALISO/path_1)"/$(cat /tmp/TIFCLEAN | head -1 | cut -d '/' -f2 | cut -d "." -f1).mov
mkdir -p "$(cat /tmp/DUALISO/path_1)"/A_ORIGINALS
mv *.{mov,MOV} "$(cat /tmp/DUALISO/path_1)"/A_ORIGINALS

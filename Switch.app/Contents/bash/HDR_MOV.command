...
#log
rm "$(cat /tmp/DUALISO/path_1)"/HDRMOV_LOG.txt
...
echo outputpath: "$(cat /tmp/DUALISO/path_1)" >> HDRMOV_LOG.txt
echo applicationpath: "$(cat /tmp/DUALISO/path_2)" >> HDRMOV_LOG.txt
echo "" >> HDRMOV_LOG.txt
...
"$(cat /tmp/DUALISO/path_2)"/ffmpeg -i 00001.tiff -pix_fmt rgb24 -vf $crp_fix 00001b.tiff &
"$(cat /tmp/DUALISO/path_2)"/ffmpeg -i 00002.tiff -pix_fmt rgb24 -vf $crp_fix 00002b.tiff &
"$(cat /tmp/DUALISO/path_2)"/ffmpeg -i 0001.tiff -pix_fmt rgb24 -vf $crp_fix 0001b.tiff &
"$(cat /tmp/DUALISO/path_2)"/ffmpeg -i 0002.tiff -pix_fmt rgb24 -vf $crp_fix 0002b.tiff &
"$(cat /tmp/DUALISO/path_2)"/ffmpeg -i 001.tiff -pix_fmt rgb24 -vf $crp_fix 001b.tiff &
"$(cat /tmp/DUALISO/path_2)"/ffmpeg -i 002.tiff -pix_fmt rgb24 -vf $crp_fix 002b.tiff &
"$(cat /tmp/DUALISO/path_2)"/ffmpeg -i 01.tiff -pix_fmt rgb24 -vf $crp_fix 01b.tiff &
"$(cat /tmp/DUALISO/path_2)"/ffmpeg -i 02.tiff -pix_fmt rgb24 -vf $crp_fix 02b.tiff &
"$(cat /tmp/DUALISO/path_2)"/ffmpeg -i 1.tiff -pix_fmt rgb24 -vf $crp_fix 1b.tiff &
"$(cat /tmp/DUALISO/path_2)"/ffmpeg -i 2.tiff -pix_fmt rgb24 -vf $crp_fix 2b.tiff &
#wait for jobs to end
    wait < <(jobs -p)
...
#check for audio
if ! [ x"$("$(cat /tmp/DUALISO/path_2)"/ffmpeg -i $(ls *.{MOV,mov,mp4,MP4,mkv,MKV,avi,AVI} | grep -v 'HDR_' | head -1) 2>&1 | grep Audio)" = x ]
then 
"$(cat /tmp/DUALISO/path_2)"/ffmpeg -i $(ls *.{MOV,mov,mp4,MP4,mkv,MKV,avi,AVI} | grep -v 'HDR_' | head -1) -vn -acodec copy $(ls *.{MOV,mov,mp4,MP4,mkv,MKV,avi,AVI} | grep -v 'HDR_' | head -1 | cut -d "." -f1).wav
...
"$(cat /tmp/DUALISO/path_2)"/ffmpeg $wav -r $(/usr/local/bin/exiftool $(ls *.{MOV,mov,mp4,MP4,mkv,MKV,avi,AVI} | grep -v 'HDR_' | head -1) | grep 'Video Frame Rate' | cut -d ":" -f2) -i %06d.tiff $acodec -vcodec prores -pix_fmt yuv422p10le ../HDR_$(cat /tmp/HDRMOVac | head -1 | cut -d "." -f1).mov 2>> /tmp/HDRMOV_LOGS/HDR_script2_LOG.txt
mv *.{MOV,mov,mp4,MP4,mkv,MKV,avi,AVI} ../
...
"$(cat /tmp/DUALISO/path_2)"/ffmpeg $wav -r $(/usr/local/bin/exiftool $(ls *.{MOV,mov,mp4,MP4,mkv,MKV,avi,AVI} | grep -v 'HDR_' | head -1) | grep 'Video Frame Rate' | cut -d ":" -f2) -i %06d.tiff $acodec -vcodec prores -pix_fmt yuv422p10le ../HDR_$(cat /tmp/HDRMOVad | head -1 | cut -d "." -f1).mov 2>> /tmp/HDRMOV_LOGS/HDR_script3_LOG.txt
mv *.{MOV,mov,mp4,MP4,mkv,MKV,avi,AVI} ../
...
sleep 1 && . /tmp/HDR_script.command | tee -a "$(cat /tmp/DUALISO/path_1)"/HDRMOV_LOG.txt &
...
chmod u=rwx "$(cat /tmp/DUALISO/path_2 | tr -d '"' )"/HDR_MOV.command

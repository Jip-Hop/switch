...
echo 2trap >> /tmp/DUALISO/prores_TRAP
#glue date
    date=$(date +%F)
...
#potential HDR settings
    ...
    then
#export ProRes proxy
    find -s "$out""$out3""$name""$sl". -maxdepth 1 -iname '*.dng' -print0 | xargs -0 dcraw +M $h2pr $opr $S -c -h -W -q 0 $gampr $wb $pix $br | ffmpeg -loglevel warning $wav2 -f image2pipe -vcodec ppm -r $fps -i pipe:0 $sd $codec -n -r $(echo $fps / 2 | bc -l) $tbl$cinpr$cinpr_01$cinpr_02$cinpr_03$cinpr_01b$cinpr_02b$cinpr_03b$scalePR$denoisePR$sharpenPR "$out""$out2""$date"_Proxy/"$name".mov 
    else
    find -s "$out""$out3""$name""$sl". -maxdepth 1 -iname '*.dng' -print0 | xargs -0 dcraw +M $h2pr $opr $S -c -h -W -q 0 $gampr $wb $pix $br | ffmpeg -loglevel warning $wav2 -f image2pipe -vcodec ppm -r "$fps" -i pipe:0 $sd $codec -n -r "$fps" $tbl$cinpr$cinpr_01$cinpr_02$cinpr_03$cinpr_01b$cinpr_02b$cinpr_03b$scalePR$denoisePR$sharpenPR "$out""$out2""$date"_Proxy/"$name".mov
    fi
    fi
    else
#tif spitter
...
#deletion of dng folders to save space
    ...
    OLDIFS=$IFS
#remove trap
find /tmp/DUALISO/prores_TRAP | xargs perl -pi -e 's/2trap//g'

...
#Processing MLV files into folders with dng files
    while ! [ x"$(cat ~/Switch/config/tmp/MLVFILESac)" = x ]
    do 
    FILE=$(cat ~/Switch/config/tmp/"MLVFILESac" | head -1 | grep -v 'avg_\|ft_')
...
#cut to the next name on the list
    echo "$(tail -n +2 ~/Switch/config/tmp/MLVFILESac)" > ~/Switch/config/tmp/MLVFILESac
    done

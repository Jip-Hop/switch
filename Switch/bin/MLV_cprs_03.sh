...
#This program is free software; you can redistribute it and/or
 ...
 # Boston, MA  02110-1301, USA.
    ...
    while ! [ x"$(cat ~/Switch/config/tmp/MLV_cprsac)" = x ]
    do 
    FILE_03="$(cat ~/Switch/config/tmp/MLV_cprsac | head -1)"
#cut to the next name on the list
    echo "$(tail -n +2 ~/Switch/config/tmp/MLV_cprsac)" > ~/Switch/config/tmp/MLV_cprsac
#run MLV compressing
    if [ -f ~/Switch/config/tmp/MLV_cprs_output ]
    then
    $mlv_dump $set -o "$(cat ~/Switch/config/tmp/MLV_cprs_output)"/tmp_"$FILE_03" "$(cat ~/Switch/config/tmp/path_1)"/"$FILE_03"
    else
...
#delete or not
    ...
    if [ -f ~/Switch/config/tmp/MLV_cprs_output ]
    then
    mv -i "$(cat ~/Switch/config/tmp/MLV_cprs_output)"/tmp_"$FILE_03" "$(cat ~/Switch/config/tmp/MLV_cprs_output)"/"$FILE_03"
    else

...
#print out trap note 1
    echo > ~/Switch/config/tmp/DF_TRAP1
    ...
    while ! [ x"$(cat ~/Switch/config/tmp/DF_storageaa)" = x ]
    do 
#get the file
    FILE_01="$(cat ~/Switch/config/tmp/DF_storageaa | head -1)"
#cut to the next name on the list
    echo "$(tail -n +2 ~/Switch/config/tmp/DF_storageaa)" > ~/Switch/config/tmp/DF_storageaa
...
#Do criterias match?
    if ls "$(cat ~/Switch/config/DARK_FOLDER)"/avg_"$bit_01"bit_"$cn_01"_res_"$res_01"_iso_"$iso_01"_fps_"$fra_01".MLV
    then
    $mlv_dump -o a_"$FILE_01" -s "$(cat ~/Switch/config/DARK_FOLDER)"/avg_"$bit_01"bit_"$cn_01"_res_"$res_01"_iso_"$iso_01"_fps_"$fra_01".MLV "$FILE_01"
    rm *MLV.wav

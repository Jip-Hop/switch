...
#This program is free software; you can redistribute it and/or
 ...
 # Boston, MA  02110-1301, USA.
    ...
    while ! [ x"$(cat ~/Switch/config/tmp/MLV_cprsad)" = x ]
    do 
    FILE_04="$(cat ~/Switch/config/tmp/MLV_cprsad | head -1)"
#cut to the next name on the list
    echo "$(tail -n +2 ~/Switch/config/tmp/MLV_cprsad)" > ~/Switch/config/tmp/MLV_cprsad
#run MLV compressing
    if [ -f ~/Switch/config/tmp/MLV_cprs_output ]
    then
    $mlv_dump $set -o "$(cat ~/Switch/config/tmp/MLV_cprs_output)"/tmp_"$FILE_04" "$(cat ~/Switch/config/tmp/path_1)"/"$FILE_04"
    else
...
#delete or not
    ...
    if [ -f ~/Switch/config/tmp/MLV_cprs_output ]
    then
    mv -i "$(cat ~/Switch/config/tmp/MLV_cprs_output)"/tmp_"$FILE_04" "$(cat ~/Switch/config/tmp/MLV_cprs_output)"/"$FILE_04"
    else

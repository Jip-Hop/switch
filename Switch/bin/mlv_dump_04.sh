...
#Processing MLV files into folders with dng files
    while ! [ x"$(cat ~/Switch/config/tmp/MLVFILESad)" = x ]
    do 
    FILE=$(cat ~/Switch/config/tmp/"MLVFILESad" | head -1 | grep -v 'avg_\|ft_')
...
#cut to the next name on the list
    echo "$(tail -n +2 ~/Switch/config/tmp/MLVFILESad)" > ~/Switch/config/tmp/MLVFILESad
    done

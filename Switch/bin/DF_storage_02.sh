...
#print out trap note 2
    echo > ~/Switch/config/tmp/DF_TRAP2
    ...
    while ! [ x"$(cat ~/Switch/config/tmp/DF_storageab)" = x ]
    do 
#get the file
    FILE_02="$(cat ~/Switch/config/tmp/DF_storageab | head -1)"
#cut to the next name on the list
    echo "$(tail -n +2 ~/Switch/config/tmp/DF_storageab)" > ~/Switch/config/tmp/DF_storageab
...
#Do criterias match?
    if ls "$(cat ~/Switch/config/DARK_FOLDER)"/avg_"$bit_02"bit_"$cn_02"_res_"$res_02"_iso_"$iso_02"_fps_"$fra_02".MLV
    then
    $mlv_dump -o a_"$FILE_02" -s "$(cat ~/Switch/config/DARK_FOLDER)"/avg_"$bit_02"bit_"$cn_02"_res_"$res_02"_iso_"$iso_02"_fps_"$fra_02".MLV "$FILE_02"
    rm *MLV.wav

...
#This program is free software; you can redistribute it and/or
 ...
 # Boston, MA  02110-1301, USA.
    ...
    while ! [ x"$(cat ~/Switch/config/tmp/MLV_cprsaa)" = x ]
    do 
    FILE_01="$(cat ~/Switch/config/tmp/MLV_cprsaa | head -1)"
#cut to the next name on the list
    echo "$(tail -n +2 ~/Switch/config/tmp/MLV_cprsaa)" > ~/Switch/config/tmp/MLV_cprsaa
#run MLV compressing
    if [ -f ~/Switch/config/tmp/MLV_cprs_output ]
    then
#safety check
    mkdir -p  "$(cat ~/Switch/config/tmp/MLV_cprs_output)"
    $mlv_dump $set -o "$(cat ~/Switch/config/tmp/MLV_cprs_output)"/tmp_"$FILE_01" "$(cat ~/Switch/config/tmp/path_1)"/"$FILE_01"
    else
...
#delete or not
    ...
    if [ -f ~/Switch/config/tmp/MLV_cprs_output ]
    then 
    mv -i "$(cat ~/Switch/config/tmp/MLV_cprs_output)"/tmp_"$FILE_01" "$(cat ~/Switch/config/tmp/MLV_cprs_output)"/"$FILE_01"
    else

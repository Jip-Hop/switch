...
#Processing MLV files into folders with dng files
    while ! [ x"$(cat ~/Switch/config/tmp/MLVFILESab)" = x ]
    do 
    FILE=$(cat ~/Switch/config/tmp/"MLVFILESab" | head -1 | grep -v 'avg_\|ft_')
...
#cut to the next name on the list
    echo "$(tail -n +2 ~/Switch/config/tmp/MLVFILESab)" > ~/Switch/config/tmp/MLVFILESab
    done

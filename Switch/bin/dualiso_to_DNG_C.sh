...
#!/bin/bash
   while grep -q 'CR2' ~/Switch/config/tmp/list_04
   do
   CR2=$(grep 'CR2' ~/Switch/config/tmp/list_04 | awk 'FNR == 1 {print}')
   DNG=$(grep 'DNG' ~/Switch/config/tmp/list_04 | awk 'FNR == 1 {print}')
#Will add stored white balance settings to processed readymade dualiso DNG files
   echo "$(tail -n +3 ~/Switch/config/tmp/list_04)" > ~/Switch/config/tmp/list_04
#check if the camera is an eos 100D
   model_match=$(exiftool "$CR2" | awk '/Camera Model Name/ { print $5,$6,$7,$8,$9,$10; exit }')
   echo "$model_match" > ~/Switch/config/tmp/model_match_C
   if grep 'Canon EOS 100D\|SL1\|Kiss X7\|X7' ~/Switch/config/tmp/model_match_C
   then

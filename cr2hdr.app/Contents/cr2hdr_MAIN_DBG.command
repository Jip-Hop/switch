...
#Create the actual darkframes
    ...
    do
#If selected deadpixel mapping
    if ls /tmp/DUALISO/DEAD_P
    then 
    mlv_dump -f 1-1 --dng "$(cat My_darkfr | head -1)" & pid_dark=$!
#wait for jobs to finish   
    sleep 2
#kill mlv_dump
    kill $pid_dark
    rm *_.wav
    dcraw -4 -T -j -t 0 -D *_000001.dng
    rm *_000001.dng
    mkdir -p $(date +%F)_ProRes4444
    deadpixels *_000001.tiff
    rm *_000001.tiff
    cat deadpixels.txt >> $(date +%F)_ProRes4444/pixel_pick
    rm deadpixels.txt
    fi
    ...
    mv "$(cat My_darkfr | head -1)" A_ORIGINALS
...
#If selected deadpixel mapping
    if ls /tmp/DUALISO/DEAD_P
    then 
    mlv_dump -f 1-1 --dng "$(cat /tmp/DUALISO/darkf.txt | head -1)" & pid_dark=$!
#wait for jobs to finish   
    sleep 2
#kill mlv_dump
    kill $pid_dark
    ...
    deadpixels *_000001.tiff
    rm *_000001.tiff
    cat deadpixels.txt >> "$(cat /tmp/DUALISO/path_1)"/$(date +%F)_ProRes4444/pixel_pick
    rm deadpixels.txt
    fi 

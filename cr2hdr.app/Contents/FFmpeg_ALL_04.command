...
echo 4trap >> /tmp/DUALISO/X_prores_TRAP
...
#determine if HDR or not
    ...
    cin=
    cinpr=
    ffmpeg -i "$name" -pix_fmt yuvj444p -r 1 -t 2 /tmp/DUALISO/check04%03d.jpeg
    convert /tmp/DUALISO/check04001.jpeg -colorspace hsb -resize 1x1 /tmp/DUALISO/check1_04.txt
    convert /tmp/DUALISO/check04002.jpeg -colorspace hsb -resize 1x1 /tmp/DUALISO/check2_04.txt
    numb=$(echo $(grep '%' /tmp/DUALISO/check1_04.txt | cut -d "%" -f3 | tr -d ',') - $(grep '%' /tmp/DUALISO/check2_04.txt | cut -d "%" -f3 | tr -d ',') | bc -l)
    if (( $(echo "$numb > 5" |bc -l) )) || (( $(echo "$numb < -5" |bc -l) )) ; then
    echo "HDR" >> /tmp/_X_HDR_04
    else
    find /tmp/_X_HDR_04 | xargs perl -pi -e 's/HDR//g'
    fi
    rm /tmp/DUALISO/check04001.jpeg
    rm /tmp/DUALISO/check04002.jpeg
    rm /tmp/DUALISO/check1_04.txt
    rm /tmp/DUALISO/check2_04.txt
...
#watch out for HDR footage
    if grep 'HDR\|halfhdr' /tmp/_X_HDR_04 
    then
    if grep 'copy' /tmp/_X_ProRes4444 
    then 
    cin= 
...
#watch ut for half fps(HDR setting)
    if grep -q 'halfhdr' /tmp/_X_HDR_04 && grep -q 'HDR' /tmp/_X_HDR_04
    then
...
#remove trap
find /tmp/DUALISO/X_prores_TRAP | xargs perl -pi -e 's/4trap//g'
